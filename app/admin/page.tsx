...
function RoleBadge({ role, translate }: { role: string; translate: (s: string, k: string) => string }) {
  ...
  return (
    <span style={{
      ...
      {isAdmin
        ? <Crown size={10} style={{ flexShrink: 0 }} />
...
function KpiCard({ label, value, color, icon: Icon, isDark }: {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: '22px', fontWeight: 500, color: color || (isDark ? '#e8f4ff' : '#0a1628'), lineHeight: 1 }}>
        {value}
      </div>
    </div>
  )
...
function StatusDot({ status }: { status: string }) {
  const map: Record<string, string> = {
    NORMAL: '#3B6D11', WARNING: '#854F0B', CRITICAL: '#A32D2D', UNKNOWN: '#5F5E5A',
  }
  return <span style={{ width: 7, height: 7, borderRadius: '50%', background: map[status] || '#5F5E5A', display: 'inline-block', marginRight: 5 }} />
}
...
function Modal({ title, children, onClose, isDark }: {
  ...
  return (
    <div onClick={e => { if (e.target === e.currentTarget) onClose() }} style={{
      ...
      <div style={{
        background: isDark ? '#1a1d2e' : 'white', borderRadius: '14px', padding: '24px',
...
export default function AdminPage() {
  ...
  const inputS: React.CSSProperties = {
    ...
    outline: 'none', background: inputBg, color: inputCol, boxSizing: 'border-box',
  ...
  const btnPrimary = (c = '#185FA5'): React.CSSProperties => ({
    ...
    padding: '7px 14px', borderRadius: '7px', background: c,
    color: 'white', border: 'none', fontSize: '12px', fontWeight: 500, cursor: 'pointer',
  })
  const btnOutline = (c = '#185FA5'): React.CSSProperties => ({
    display: 'inline-flex', alignItems: 'center', gap: '6px',
    padding: '7px 14px', borderRadius: '7px', background: 'transparent',
    color: c, border: `0.5px solid ${c}`, fontSize: '12px', fontWeight: 500, cursor: 'pointer',
  })
  ...
  const critNodes  = nodes.length  // simplifié — adapter si statut dispo
  ...
  const handleCreateNode = async () => {
    ...
    try {
      ...
      setShowCreateNode(false); setNewNode({ name: '', role: 'CCN', description: '', ip_address: '', server_type: '', port: '' }); fetchNodes()
    ...
    <DashboardLayout>
      ...
      {toast && (
        <div style={{
          ...
          fontSize: '12px', fontWeight: 500, boxShadow: '0 8px 24px rgba(0,0,0,0.3)',
      ...
      <div style={{ display: 'flex', gap: '20px', minHeight: '80vh' }}>
        ...
        {/* ── Sidebar admin ── */}
        <aside style={{
          width: '200px', flexShrink: 0, background: cardBg,
          border: `0.5px solid ${borderCol}`, borderRadius: '12px',
          ...
          {navItems.slice(0, 2).map(item => (
            <NavItem key={item.key} item={item} active={view === item.key}
              onClick={() => setView(item.key)} textPri={textPri} textSec={textSec} isDark={isDark} />
          ))}
                  ...
                  {loadingN ? <div style={{ color: textSec, fontSize: '12px' }}>Chargement…</div> : nodes.slice(0, 6).map(n => (
                    <div key={n.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 0', borderBottom: `0.5px solid ${borderCol}` }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: '12px', fontWeight: 500, color: textPri }}>{n.display_name || n.name}</div>
                        <div style={{ fontSize: '10px', color: textSec }}>{n.name}</div>
                      </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
                  ...
                  {(['all', 'admin', 'operator', 'inactive'] as const).map(f => (
                    <button key={f} onClick={() => setUserFilter(f)} style={{
                      padding: '4px 10px', borderRadius: '14px', fontSize: '11px', cursor: 'pointer',
                      background: userFilter === f ? '#185FA5' : 'transparent',
                      color: userFilter === f ? 'white' : textSec,
                      border: `0.5px solid ${userFilter === f ? '#185FA5' : borderCol}`,
                      fontWeight: userFilter === f ? 500 : 400,
                    }}>
                            ...
                            <tr key={u.id} style={{ borderBottom: `0.5px solid ${borderCol}` }}
                              ...
                              <td style={{ padding: '10px 12px' }}>
                                {u.role === 'admin'
                                  ...
                                  : <span style={{ fontSize: '11px', fontWeight: 500, color: expiry.color, background: expiry.bg, padding: '2px 8px', borderRadius: '10px' }}>{expiry.label}</span>
                                }
                              </td>
                              ...
                              <td style={{ padding: '10px 12px' }}>
                                <div style={{ display: 'flex', gap: '5px' }}>
                                  ...
                                  {u.role === 'operator' && (
                                    <ActionBtn onClick={() => openExpiry(u)} color="#534AB7" title={t('admin','expiration')}><Clock size={12} /></ActionBtn>
                                  )}
                                  <ActionBtn onClick={() => { setResetTarget(u); setNewPwd('') }} color="#854F0B" title={t('admin','resetPassword')}><KeyRound size={12} /></ActionBtn>
                                  {u.username !== 'admin' && (
                                    <ActionBtn onClick={() => handleDelete(u)} color="#A32D2D" title={t('admin','delete')}><Trash2 size={12} /></ActionBtn>
                                  )}
                                </div>
                              </td>
                            </tr>
                        ...
                        <tr key={n.id} style={{ borderBottom: `0.5px solid ${borderCol}` }}
                          ...
                          <td style={{ padding: '10px 12px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              ...
                              <div>
                                <div style={{ fontSize: '12px', fontWeight: 500, color: textPri }}>{n.display_name || n.name}</div>
                                <div style={{ fontSize: '10px', color: textSec, fontFamily: 'monospace' }}>{n.name}</div>
                              </div>
                            </div>
                          </td>
                          <td style={{ padding: '10px 12px' }}><NodeTypeBadge role={n.role} /></td>
      ...
      {showCreate && (
        <Modal title={t('admin', 'createUser')} onClose={() => { setShowCreate(false); setCreateErr('') }} isDark={isDark}>
          ...
          <Field label={t('admin', 'emailLabel')}>
            <input style={inputS} type="email" placeholder="Ex : ghassen@tunisietelecom.tn" value={newUser.email} onChange={e => setNewUser(p => ({ ...p, email: e.target.value }))} />
          </Field>
        ...
        <Modal title={t('admin', 'resetPassword')} onClose={() => { setResetTarget(null); setNewPwd('') }} isDark={isDark}>
          ...
          <Field label={t('admin', 'newPassword')}>
            <div style={{ position: 'relative' }}>
              ...
              <button onClick={() => setShowNewPwd(p => !p)} style={{ position: 'absolute', right: '10px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: textSec, display: 'flex' }}>
        ...
        <Modal title={t('admin', 'changePassword')} onClose={() => { setShowChangePwd(false); setChangePwdErr('') }} isDark={isDark}>
          {[
            ...
            <Field key={f.key} label={f.label}>
              <div style={{ position: 'relative' }}>
                <input style={{ ...inputS, paddingRight: '40px' }} type={f.show ? 'text' : 'password'} placeholder="••••••••"
                  value={f.val} onChange={e => setChangePwdForm(p => ({ ...p, [f.key]: e.target.value }))} />
                <button onClick={f.toggle} style={{ position: 'absolute', right: '10px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: textSec, display: 'flex' }}>
      ...
      {editNode && (
        <Modal title={`${t('admin','modify')} — ${editNode.name}`} onClose={() => { setEditNode(null); setEditNodeErr('') }} isDark={isDark}>
          ...
          {[
            ...
            <Field key={f.key} label={f.label}>
              <input style={inputS} placeholder={f.placeholder}
...
function NavItem({ item, active, onClick, textPri, textSec, isDark }: {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      padding: '7px 10px', borderRadius: '7px', width: '100%',
      background: active ? 'rgba(24,95,165,0.12)' : 'transparent',
      color: active ? '#185FA5' : textSec,
      ...
      {item.count !== undefined && (
        <span style={{ fontSize: '10px', fontWeight: 500, padding: '1px 6px', borderRadius: '10px', background: active ? '#185FA5' : (isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)'), color: active ? 'white' : textSec }}>
          {item.count}
        </span>
      )}
    </button>
