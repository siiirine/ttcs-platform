...
interface HistoryItem { question: string; reponse: string; created_at: string }

function getUsername(): string {
  try {
    for (const cookie of document.cookie.split(';')) {
      const [name, value] = cookie.trim().split('=')
      if (name === 'ttcs_user') return JSON.parse(decodeURIComponent(value)).username || 'admin'
    }
  } catch { }
  return 'admin'
}
...
export default function AssistantPage() {
  ...
  const initMessages = (): Message[] => [{ id: '1', role: 'assistant', timestamp: new Date(), content: t('assistant', 'welcome') }]
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/chat/history/${getUsername()}?limit=30`, { headers: { Authorization: `Bearer ${getToken()}` } })
  ...
  const loadFromHistory = (item: HistoryItem, index: number) => {
    ...
    setMessages([
      { id: '1',  role: 'assistant', timestamp: new Date(),               content: t('assistant', 'welcome') },
    ...
    setGroqHistory([{ role: 'user', content: item.question }, { role: 'assistant', content: item.reponse }])
  }

  const newConversation = () => { setActiveHistory(null); setMessages(initMessages()); setGroqHistory([]) }
  ...
  const handleSend = async () => {
    ...
    const question = input.trim(); setInput(''); setIsLoading(true)
    try {
      ...
      setMessages(prev => [...prev, { id: (Date.now() + 1).toString(), role: 'assistant', content: t('assistant', 'connectionError'), timestamp: new Date() }])
    ...
    <DashboardLayout>
      ...
      <div style={{ height: 'calc(100vh - 64px)', display: 'flex' }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          <div style={{ padding: '16px 24px', borderBottom: `1px solid ${c.borderSubtle}`, background: c.headerBg, display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{ marginLeft: 'auto', padding: '4px 12px', borderRadius: '20px', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.3)', fontSize: '12px', color: '#10b981', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '6px' }}>
              <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#10b981' }} /> {t('assistant', 'online')}
            </div>
          </div>

          <div style={{ flex: 1, overflowY: 'auto', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {messages.map(message => (
              <MessageBubble key={message.id} message={message} cardBg={c.cardBg} textPrimary={c.textPrimary} textSecondary={c.textSecondary} border={c.border} />
            ))}
            {isLoading && (
          ...
          <div style={{ padding: '16px 24px', borderTop: `1px solid ${c.borderSubtle}`, background: c.headerBg }}>
            <div style={{ display: 'flex', gap: '12px' }}>
              <input type="text" value={input} onChange={e => setInput(e.target.value)}
                onKeyPress={e => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), handleSend())}
                placeholder={t('assistant', 'placeholder')} disabled={isLoading}
                style={{ flex: 1, padding: '13px 18px', borderRadius: '12px', border: `1.5px solid ${c.borderInput}`, background: c.inputBg, fontSize: '14px', color: c.textPrimary, outline: 'none' }}
        ...
        <div style={{ width: '280px', flexShrink: 0, borderLeft: `1px solid ${c.borderSubtle}`, background: c.sidebarRightBg, display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '16px', borderBottom: `1px solid ${c.borderSubtle}` }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <History size={15} style={{ color: '#0082f0' }} />
                <span style={{ fontSize: '13px', fontWeight: 700, color: c.textPrimary }}>{t('assistant', 'history')}</span>
              </div>
              <span style={{ fontSize: '10px', color: c.textSecondary, background: 'rgba(0,130,240,0.1)', padding: '2px 7px', borderRadius: '999px', fontWeight: 600 }}>{history.length}</span>
            </div>
            <button onClick={newConversation} style={{ width: '100%', padding: '8px 12px', borderRadius: '8px', background: 'linear-gradient(135deg, #0055cc, #0082f0)', color: 'white', border: 'none', fontSize: '12px', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
              <Plus size={13} /> {t('assistant', 'newConversation')}
            </button>
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: '8px' }}>
            {loadingHistory ? (
              <div style={{ textAlign: 'center', padding: '20px', color: c.textSecondary, fontSize: '12px' }}>{t('assistant', 'loading')}</div>
            ) : history.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '24px 16px', color: c.textSecondary }}>
                <History size={28} style={{ opacity: 0.3, marginBottom: '8px' }} />
                <p style={{ fontSize: '12px', margin: 0 }}>{t('assistant', 'noConversation')}</p>
              </div>
            ) : (
              history.map((item, i) => (
                <button key={i} onClick={() => loadFromHistory(item, i)} style={{ width: '100%', textAlign: 'left', padding: '10px 12px', borderRadius: '8px', marginBottom: '4px', border: 'none', background: activeHistory === i ? 'rgba(0,130,240,0.12)' : 'transparent', cursor: 'pointer', transition: 'all 0.15s', borderLeft: activeHistory === i ? '3px solid #0082f0' : '3px solid transparent' }}
                  ...
                  <p style={{ fontSize: '12px', fontWeight: 500, color: c.textPrimary, margin: '0 0 3px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.question}</p>
                  <p style={{ fontSize: '11px', color: c.textSecondary, margin: '0 0 4px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.reponse.replace(/\*\*/g, '').substring(0, 55)}...</p>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <Clock size={10} style={{ color: c.textSecondary, opacity: 0.6 }} />
                    <span style={{ fontSize: '10px', color: c.textSecondary, opacity: 0.7 }}>{formatRelativeTime(item.created_at)}</span>
                  </div>
                </button>
              ))
            )}
...
function MessageBubble({ message, cardBg, textPrimary, textSecondary, border }: { message: Message; cardBg: string; textPrimary: string; textSecondary: string; border: string }) {
  const isAssistant = message.role === 'assistant'
  const renderContent = (content: string) => content.split('\n').map((line, i) => {
    const parts = line.split(/\*\*(.*?)\*\*/g)
    return <p key={i} style={{ margin: i > 0 ? '4px 0 0' : '0' }}>{parts.map((part, j) => j % 2 === 1 ? <strong key={j} style={{ color: isAssistant ? '#0082f0' : 'white', fontWeight: 700 }}>{part}</strong> : part)}</p>
  })
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', flexDirection: isAssistant ? 'row' : 'row-reverse' }}>
      ...
      <div style={{ maxWidth: '75%', padding: '14px 18px', borderRadius: isAssistant ? '4px 16px 16px 16px' : '16px 4px 16px 16px', background: isAssistant ? cardBg : 'linear-gradient(135deg, #0055cc, #0082f0)', border: isAssistant ? `1px solid ${border}` : 'none', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
        <div style={{ fontSize: '14px', lineHeight: 1.6, color: isAssistant ? textPrimary : 'white', whiteSpace: 'pre-wrap' }}>{renderContent(message.content)}</div>
