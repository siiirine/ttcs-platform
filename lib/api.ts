...
export interface SummaryResponse {
  ...
  critical_nodes: string[]; warning_nodes: string[]
}
export interface HardwareStatus {
  status: 'CRITICAL'|'WARNING'|'NORMAL'; cpu_pct: number; ram_pct: number
  cpu_temp: number; ups_pct: number; fan_rpm: number; io_latency: number
  watts: number; rail_12v: number; issues: string[]
}
export interface OSStatus {
  status: 'CRITICAL'|'WARNING'|'NORMAL'; zombie_procs: number; fd_used: number
  tcp_retrans_rate: number; oom_kills: number; ntp_offset_ms: number
  load_avg_1m: number; load_avg_5m: number; load_avg_15m: number
  uptime_sec: number; issues: string[]
}
export interface AppStatus {
  status: 'CRITICAL'|'WARNING'|'NORMAL'; down_services: string[]
  critical_alarms: number; major_alarms: number; sdp_timeout_count: number
  sdp_conflict_count: number; ccn_lookup_fail: number; ccn_comm_error: number
  occ_alarm_state: number; issues: string[]
}
export interface CIPStatus {
  status: 'CRITICAL'|'WARNING'|'NORMAL'; last_fail_rate: number
  last_reject_rate: number; z_score_fail: number; z_score_reject: number; issues: string[]
}
export interface NodeStatus {
  role: string; global_status: 'CRITICAL'|'WARNING'|'NORMAL'
...
export interface TimelineResponse {
  node: string; role: string; global_status: 'CRITICAL'|'WARNING'|'NORMAL'
  timestamp: string; events: TimelineEvent[]; nb_anomalies: number
}
export interface InventoryNode { role: string; description: string; port: number }
...
export const adminApi = {
  ...
  deleteNode:  (name: string) =>
    fetchAPI<{ message: string }>(`/admin/nodes/${name}`, { method: 'DELETE' }),
}

export const KNOWN_NODES = ['jambala', 'ttair6', 'ttsdp17a', 'ttvs3a', 'ttocc1', 'ttaf1']
export const ROLE_COLORS: Record<string, string> = {
  CCN: 'from-blue-500 to-blue-600',   SDP: 'from-purple-500 to-purple-600',
  OCC: 'from-orange-500 to-orange-600', AIR: 'from-teal-500 to-teal-600',
  VS:  'from-slate-500 to-slate-600',  AF:  'from-pink-500 to-pink-600',
}
export const ROLE_DESCRIPTIONS: Record<string, string> = {
  CCN: 'Charging Control Node',       SDP: 'Service Data Point / TimesTen',
  OCC: 'Online Charging Center',      AIR: 'Account Information & Reservation',
  VS:  'Voucher Server',              AF:  'Account Filter / DNS',
}