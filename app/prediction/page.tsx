...
const NODE_DISPLAY_NAMES: Record<string, string> = {
  'jambala': 'CCN-Node-01', 'ttair6': 'AIR-Node-01', 'ttsdp17a': 'SDP-Node-01',
  'ttvs3a': 'VS-Node-01',  'ttocc1': 'OCC-Node-01',  'ttaf1': 'AF-Node-01',
}
...
export default function PredictionPage() {
  ...
  const fetchData = useCallback(async () => {
    try { const data = await api.getPredict(); setPredictions(data); setError(null) }
    catch (err) { setError(t('prediction','loadError')); console.error('API Error:', err) }
    finally { setIsLoading(false) }
  }, [t])

  const handleRefresh = async () => {
    setIsRefreshing(true)
    try { await api.refreshPredict(); await fetchData() }
    ...
    <DashboardLayout><Topbar />
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 'calc(100vh - 64px)' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
          ...
          <p style={{ color: c.textSecondary }}>{t('prediction','loading')}</p>
        </div>
      </div>
    </DashboardLayout>
  )
    ...
    <DashboardLayout><Topbar />
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 'calc(100vh - 64px)' }}>
        <div style={{ background: c.cardBg, border: '1px solid rgba(239,68,68,0.3)', borderRadius: '16px', padding: '32px', maxWidth: '400px', textAlign: 'center' }}>
          ...
          <h2 style={{ color: c.textPrimary, fontWeight: 700, marginBottom: '8px' }}>{t('prediction','loadError')}</h2>
          <p style={{ color: c.textSecondary, marginBottom: '24px' }}>{error}</p>
          <button onClick={fetchData} style={{ background: 'linear-gradient(135deg, #0082f0, #00d4aa)', color: 'white', border: 'none', borderRadius: '10px', padding: '10px 24px', fontWeight: 600, cursor: 'pointer' }}>{t('prediction','retry')}</button>
        </div>
      </div>
    </DashboardLayout>
  )
      ...
      <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '5fr 7fr', gap: '16px' }}>
          <div style={{ background: c.cardBg, border: `1px solid ${c.border}`, borderRadius: '14px', padding: '24px', boxShadow: c.shadow }}>
            <p style={{ fontSize: '11px', fontWeight: 700, color: c.textSecondary, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '20px' }}>{t('prediction','globalRisk')}</p>
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <RiskGauge score={riskScore} level={predictions?.global.risk_level || 'NORMAL'} textColor={c.textPrimary} subColor={c.textSecondary} riskLabel={t('prediction','risk')} />
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ background: c.cardBg, border: `1px solid ${c.border}`, borderRadius: '14px', padding: '20px', boxShadow: c.shadow }}>
              <p style={{ fontSize: '11px', fontWeight: 700, color: c.textSecondary, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '10px' }}>{t('prediction','interpretation')}</p>
              <p style={{ fontSize: '15px', color: c.textPrimary, lineHeight: 1.6 }}>{predictions?.global.interpretation || t('prediction','analyzing')}</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div style={{ background: c.cardBg, border: `1px solid ${c.border}`, borderRadius: '14px', padding: '20px', boxShadow: c.shadow }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                  ...
                  <p style={{ fontSize: '11px', fontWeight: 700, color: c.textSecondary, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{t('prediction','sdpAnalyzed')}</p>
                </div>
              ...
              <div style={{ background: c.cardBg, border: `1px solid ${c.border}`, borderRadius: '14px', padding: '20px', boxShadow: c.shadow, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <button onClick={handleRefresh} disabled={isRefreshing} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px', background: 'none', border: 'none', cursor: 'pointer' }}>
                  ...
                  <p style={{ fontSize: '13px', fontWeight: 600, color: c.textPrimary }}>{isRefreshing ? t('prediction','refreshing') : t('prediction','refresh')}</p>
                </button>
              </div>
            </div>
          </div>
...
function RiskGauge({ score, level, textColor, subColor, riskLabel }: { score: number; level: string; textColor: string; subColor: string; riskLabel: string }) {
  ...
  const gradId = `rg-${level}`, c = colors[level] || colors.NORMAL
  return (
    <div style={{ position: 'relative', width: '200px', height: '200px' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <span style={{ fontSize: '11px', color: subColor, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{riskLabel}</span>
      </div>
    </div>
  )
}
...
function SDPNodeCard({ name, sdp }: { name: string; sdp: SDPPrediction }) {
  ...
  const chartData = sdp.prediction.predictions.map(p => ({ label: p.label, failRate: +(p.fail_rate * 100).toFixed(4) }))
  ...
  return (
    <div style={{ background: c.cardBg, border: `1px solid ${c.border}`, borderTop: `4px solid ${borderColors[sdp.prediction.risk_level]}`, borderRadius: '14px', overflow: 'hidden', boxShadow: c.shadow }}>
      <div style={{ padding: '16px 24px', borderBottom: `1px solid ${c.borderSubtle}`, background: c.panelBg, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '11px', color: c.textSecondary }}>{t('prediction','serviceDataPoint')}</p>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '4fr 8fr' }}>
        <div style={{ padding: '20px', borderRight: `1px solid ${c.borderSubtle}` }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '16px' }}>
            {[
              { label: t('prediction','currentFailRate'), value: `${(sdp.prediction.current_fail_rate * 100).toFixed(4)}%`,          color: c.textPrimary },
              { label: t('prediction','meanPredicted'),   value: `${(sdp.prediction.mean_predicted_fail_rate * 100).toFixed(4)}%`,    color: c.textPrimary },
              { label: t('prediction','zScore'),          value: sdp.prediction.z_score_actuel.toFixed(3),                            color: Math.abs(sdp.prediction.z_score_actuel) > 2 ? '#ef4444' : c.textPrimary },
              { label: t('prediction','horizon'),         value: sdp.prediction.horizon,                                              color: '#0082f0' },
          ...
          <div style={{ background: c.panelBg, borderRadius: '10px', padding: '12px', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px', border: `1px solid ${c.borderSubtle}` }}>
            ...
            <div>
              <p style={{ fontSize: '10px', fontWeight: 700, color: c.textSecondary, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{t('prediction','trend')}</p>
          ...
          <div style={{ background: c.panelBg, borderRadius: '10px', padding: '12px', border: `1px solid ${c.borderSubtle}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '10px' }}>
              ...
              <p style={{ fontSize: '10px', fontWeight: 700, color: c.textSecondary, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{t('prediction','isolationForest')}</p>
            </div>
            {[
              { label: t('prediction','historicalAnomaly'), value: sdp.isolation_forest.nb_anomalies_historiques },
              { label: t('prediction','anomalyRate'),       value: `${(sdp.isolation_forest.taux_anomalie * 100).toFixed(2)}%` },
              { label: t('prediction','meanScore'),         value: sdp.isolation_forest.score_moyen.toFixed(4) },
            ].map(({ label, value }) => (
        ...
        <div style={{ padding: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
            ...
            <p style={{ fontSize: '11px', fontWeight: 700, color: c.textSecondary, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{t('prediction','predictionOn')} {sdp.prediction.horizon}</p>
          </div>
          <div style={{ height: '260px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <Area type="monotone" dataKey="failRate" stroke="#0082f0" strokeWidth={2.5} fill={`url(#cg-${name})`} dot={{ fill: '#0082f0', strokeWidth: 2, r: 3 }} activeDot={{ r: 5, fill: '#00d4aa' }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          <div style={{ marginTop: '16px', paddingTop: '12px', borderTop: `1px solid ${c.borderSubtle}`, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
            {[
              { label: t('prediction','analyzedPoints'), value: sdp.statistiques.nb_points.toLocaleString() },
              { label: t('prediction','meanTotal'),      value: sdp.statistiques.total_moyen.toLocaleString() },
              { label: t('prediction','meanFailRate'),   value: `${(sdp.statistiques.fail_rate_moyen * 100).toFixed(4)}%` },
              { label: t('prediction','maxFailRate'),    value: `${(sdp.statistiques.fail_rate_max * 100).toFixed(4)}%` },
            ].map(({ label, value }) => (
              <div key={label}>
